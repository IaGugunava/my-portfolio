  ...
  <NuxtLink :to="data?.link" target="_blank" class="group">
    <div class="relative w-full overflow-hidden rounded-lg">
      <NuxtImg
        ...
        :class="{'group-hover:scale-125' : data?.link}"
      />

      <div v-if="!data?.link" class="absolute top-0 left-0 w-full h-full bg-primary-dark bg-opacity-30 font-semibold transition-opacity duration-500 ease-in-out opacity-0 group-hover:opacity-100 flex justify-center items-center text-white text-2xl">
        Link not available
      </div>
    </div>
    <div>
      ...
      <div v-if="technologies?.length" class="pl-2">
        <swiper-container
          :breakpoints="{
            0: {
              spaceBetween: 8,
